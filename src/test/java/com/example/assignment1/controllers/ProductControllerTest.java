...
class ProductControllerTest {
    ...
    private void initialFullData() {
        ...
        when(productRepository.findByName("")).thenReturn(entities);
    }

    @Test
    void search_with_nullKeyword() {
        // Arrange
        initialFullData();
        ProductFilter filter = new ProductFilter(0, 0, null);

        // Act
        BaseSearchResultModel<ProductInfo> result = testRestTemplate.postForObject(
                "/product/search",
                filter,
                BaseSearchResultModel.class);
