...
import com.example.assignment1.database.UserAddress;
...
@DataJpaTest
class UserBasketRepositoryTest {
    ...
    private void initialUserData() {
        ...
        UserAddress address1 = new UserAddress(user, "ต.ลำลูกกา อ.ลำลูกกา จ.ปทุมธานี 12150");
        UserAddress address2 = new UserAddress(user, "แขวงหนองจอก เขตหนองจอก กรุงเทพมหานคร 10530");
        ...
        // Get UserId
        List<User> findUserQueryResult = userRepository.findByUsername(user.getUsername());
        if (!findUserQueryResult.isEmpty()) {
            userId = findUserQueryResult.get(0).getId();
        }
    }
    ...
    @Test
    @DisplayName("ส่ง Product มา 1 รายการ (ProductId = 1)")
    void addToBasket_with_singleProduct() {
        // Arrange
        initialData();
        UserBasket userBasket = new UserBasket(userId, 1, 1);
        int userBasketSizeBefore = userBasketRepository.findAll().size();

        // Act
        userBasketRepository.save(userBasket);

        // Assert
        List<UserBasket> allUserBasket = userBasketRepository.findAll();
        assertEquals(0, userBasketSizeBefore);
        assertFalse(allUserBasket.isEmpty());
        assertEquals(1, allUserBasket.size());
        assertEquals(userId, allUserBasket.get(0).getUserId());
        assertEquals(1, allUserBasket.get(0).getProductId());
        assertEquals(1, allUserBasket.get(0).getAmount());
    }

    @Test
    @DisplayName("ส่ง Product มามากกว่า 1 รายการ (ProductId = [5, 6])")
    void addToBasket_with_multiProduct() {
        // Arrange
        initialData();
        List<UserBasket> userBaskets = new ArrayList<>();
        UserBasket userBasket1 = new UserBasket(userId, 5, 3);
        UserBasket userBasket2 = new UserBasket(userId, 6, 9);
        userBaskets.add(userBasket1);
        userBaskets.add(userBasket2);

        // Act
        userBasketRepository.saveAll(userBaskets);

        // Assert
        List<UserBasket> allUserBasket = userBasketRepository.findAll();
        assertFalse(allUserBasket.isEmpty());
        assertEquals(2, allUserBasket.size());

        assertEquals(userId, allUserBasket.get(0).getUserId());
        assertEquals(5, allUserBasket.get(0).getProductId());
        assertEquals(3, allUserBasket.get(0).getAmount());

        assertEquals(userId, allUserBasket.get(1).getUserId());
        assertEquals(6, allUserBasket.get(1).getProductId());
        assertEquals(9, allUserBasket.get(1).getAmount());
    }

    @Test
    @DisplayName("ส่ง Product มา 1 รายการ (ProductId = 2), ไม่มีข้อมูลเดิม, สร้าง Record ใหม่ 1 รายการ")
    void addToBasket_with_singleProduct_newEntity() {

    }

    @Test
    @DisplayName("ส่ง Product มาหลายรายการ (ProductId = 12, 15), ไม่มีข้อมูลเดิม, สร้าง Record ใหม่ 2 รายการ")
    void addToBasket_with_multiProduct_newEntity() {

    }

    @Test
    @DisplayName("ส่ง Product มา 1 รายการ (ProductId = 8), มีข้อมูลเดิม, " +
            "ทำการ Update Record นั้นด้วย Amount จาก Request + Amount เดิมจาก UserBasket Record")
    void addToBasket_with_singleProduct_updateEntity() {
        // Arrange
        UserBasket entity = new UserBasket(1L, 8L, 5);
        userBasketRepository.save(entity);

        List<UserBasket> updateEntities = userBasketRepository.findAll();
        UserBasket updateEntity = new UserBasket();
        if (!updateEntities.isEmpty()) {
            updateEntity = updateEntities.get(0);
        }

        updateEntity.setAmount(20);

        // Act
        userBasketRepository.save(updateEntity);

        // Assert
        assertEquals(1, userBasketRepository.findAll().size());
        assertEquals(20, userBasketRepository.findAll().get(0).getAmount());
        assertEquals(8L, userBasketRepository.findAll().get(0).getProductId());
    }

    @Test
    @DisplayName("ส่ง Product มาหลายรายการ (ProductId = 96, 73), มีข้อมูลเดิม 2 รายการ, " +
            "ทำการ Update Record นั้นด้วย Amount จาก Request ทั้ง 2 รายการ")
    void addToBasket_with_multiProduct_updateBothEntity() {

    }

    @Test
    @DisplayName("ส่ง Product มาหลายรายการ (ProductId = 156, 178), มีข้อมูลเดิม 1 รายการ (ProductId = 178), " +
            "ทำการ Update Record นั้นด้วย Amount จาก Request 1 รายการ, " +
            "สร้าง Record ใหม่ (ProductId = 156) 1 รายการ")
    void addToBasket_with_multiProduct_updateSingleEntity_newSingleEntity() {
